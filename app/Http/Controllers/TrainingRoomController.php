...
class TrainingRoomController extends Controller
{
    ...
    //Training Room Folder-Start
    public function index()
    {
        $page = "training_room";
        $Role = Session::get('user_role');
        return view('dashboard.training-room.index', compact('page', 'Role'));
    }
    ...
    public function OpenTrainingRoomFolders($RoleId)
    {
        ...
        $TrainingRoomRole = DB::table('roles')
            ->where('id', '=', $RoleId)
            ->get()[0]->title;
        return view('dashboard.training-room.folders', compact('page', 'Role', 'RoleId', 'TrainingRoomRole'));
    }
    ...
    public function AddTrainingRoomFolder($RoleId)
    {
        ...
        return view('dashboard.training-room.add-new-folder', compact('page', 'Role', 'RoleId', 'TrainingRoomRoleId'));
    }
    ...
    public function StoreTrainingRoomFolder(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('message', 'Folder has been added into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    }
    ...
    public function LoadAllTrainingRoomFolders(Request $request)
    {
        ...
        foreach ($fetch_data as $row => $item) {
            $sub_array = array();
            $sub_array['id'] = $SrNo;
            $sub_array['name'] = '<span>' . wordwrap($item->name, 40, '<br>') . '</span>';
            ...
            $Url = url('training-room/folder/order/up') . '/' . $item->id . '/' . $item->role_id;
            $Url2 = url('training-room/folder/order/down') . '/' . $item->id . '/' . $item->role_id;
            $sub_array['action'] = '<button class="btn btn-primary mr-2" onclick="window.location.href=\'' . url('training-room/folder/details/' . $item->id . '/' . $TrainingRoomRoleId) . '\'" data-toggle="tooltip" title="Open Folder"><i class="fas fa-eye"></i></button><button class="btn btn-primary mr-2" onclick="window.location.href=\'' . url('training-room/folder/edit/' . $item->id . '/' . $TrainingRoomRoleId) . '\'" data-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-primary mr-2" id="delete_' . $item->id . '" onclick="deleteTrainingRoomFolder(this.id);" data-toggle="tooltip" title="Delete"><i class="fas fa-trash"></i></button><button class="btn btn-primary mr-2" id="copy_' . $item->id . '" onclick="copyTrainingRoomFolder(this.id);" data-toggle="tooltip" title="Copy"><i class="fas fa-copy"></i></button><button class="btn btn-primary mr-2" id="orderUp_' . $item->id . '" onclick="window.location.href=\'' . $Url . '\';" data-toggle="tooltip" title="Order Up"><i class="fas fa-arrow-up"></i></button><button class="btn btn-primary mr-2" id="orderDown_' . $item->id . '" onclick="window.location.href=\'' . $Url2 . '\';" data-toggle="tooltip" title="Order Down"><i class="fas fa-arrow-down"></i></button>';
            $SrNo++;
            $data[] = $sub_array;
        }
    ...
    public function DeleteTrainingRoomFolder(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('message', 'Folder has been removed from the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    function CopyTrainingRoomFolder(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('message', 'Folder has been copied from the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    public function EditTrainingRoomFolder($FolderId, $RoleId)
    {
        ...
        return view('dashboard.training-room.edit-folder', compact('page', 'Role', 'RoleId', 'folder'));
    }
    ...
    public function UpdateTrainingRoomFolder(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('message', 'Folder has been updated into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folders/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    function TrainingRoomFolderOrderUp($Id, $Role){
        ...
        return redirect(url('training-room/folders/' . $Role))->with('message', 'Record order changed successfully');
    }
    ...
    public function OpenTrainingRoomDetails($FolderId, $RoleId)
    {
        ...
        $TrainingRoomRole = DB::table('roles')
            ->where('id', '=', $RoleId)
            ->get()[0]->title;
        return view('dashboard.training-room.details', compact('page', 'FolderId', 'Role', 'RoleId', 'TrainingRoomRole'));
    }

    //Training Room Video-Start
    public function AddTrainingRoomVideo($FolderId, $RoleId)
    {
        $page = "training_room";
        $Role = Session::get('user_role');
        $TrainingRoomRoleId = $RoleId;
        return view('dashboard.training-room.add-new-video', compact('page', 'FolderId', 'Role', 'RoleId', 'TrainingRoomRoleId'));
    }
    ...
    public function TrainingRoomVideoStore(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Video has been added into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    }
    ...
    public function EditTrainingRoomVideo($VideoId, $FolderId, $RoleId)
    {
        ...
        return view('dashboard.training-room.edit-video', compact('page', 'FolderId', 'Role', 'RoleId', 'video'));
    }
    ...
    public function TrainingRoomVideoUpdate(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Video has been updated into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    //Training Room Article-Start
    public function AddTrainingRoomArticle($FolderId, $RoleId)
    {
        $page = "training_room";
        $Role = Session::get('user_role');
        $TrainingRoomRoleId = $RoleId;
        return view('dashboard.training-room.add-new-article', compact('page', 'FolderId', 'Role', 'RoleId', 'TrainingRoomRoleId'));
    }
    ...
    public function TrainingRoomArticleStore(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Article has been added into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    public function EditTrainingRoomArticle($ArticleId, $FolderId, $RoleId)
    {
        ...
        return view('dashboard.training-room.edit-article', compact('page', 'FolderId', 'Role', 'RoleId', 'article'));
    }
    ...
    public function TrainingRoomArticleUpdate(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Article has been updated into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    //Training Room Quiz-Start
    public function AddTrainingRoomQuiz($FolderId, $RoleId)
    {
        $page = "training_room";
        $Role = Session::get('user_role');
        $TrainingRoomRoleId = $RoleId;
        return view('dashboard.training-room.add-new-quiz', compact('page', 'FolderId', 'Role', 'RoleId', 'TrainingRoomRoleId'));
    }
    ...
    public function TrainingRoomQuizStore(Request $request)
    {
        ...
        if ($Affected && $Affected2) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Quiz has been added into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    public function EditTrainingRoomQuiz($QuizId, $FolderId, $RoleId)
    {
        ...
        $Data[] = $RoleId;
        $Data[] = $Questions;
        return view('dashboard.training-room.edit-quiz', compact('page', 'FolderId', 'Role', 'RoleId', 'quiz', 'Data'));
    }
    ...
    public function TrainingRoomQuizUpdate(Request $request)
    {
        ...
        if ($Affected && $Affected2) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Quiz has been updated into the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    public function LoadAllTrainingRoom(Request $request)
    {
        ...
        foreach ($fetch_data as $row => $item) {
            ...
            $Url = url('training-room/order/up') . '/' . $item->id . '/' . $item->folder_id . '/' . $item->role_id;
            $Url2 = url('training-room/order/down') . '/' . $item->id . '/' . $item->folder_id . '/' . $item->role_id;
            if ($item->type == "video") {
                $sub_array['action'] = '<button class="btn btn-primary mr-2" onclick="window.location.href=\'' . url('training-room/video/edit/' . $item->id . '/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId) . '\'" data-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-primary mr-2" id="delete_' . $item->id . '" onclick="deleteTrainingRoom(this.id);" data-toggle="tooltip" title="Delete"><i class="fas fa-trash"></i></button><button class="btn btn-primary mr-2" id="copy_' . $item->id . '" onclick="copyTrainingRoomItem(this.id);" data-toggle="tooltip" title="Copy"><i class="fas fa-copy"></i></button><button class="btn btn-primary mr-2" id="orderUp_' . $item->id . '" onclick="window.location.href=\'' . $Url . '\';" data-toggle="tooltip" title="Order Up"><i class="fas fa-arrow-up"></i></button><button class="btn btn-primary mr-2" id="orderDown_' . $item->id . '" onclick="window.location.href=\'' . $Url2 . '\';" data-toggle="tooltip" title="Order Down"><i class="fas fa-arrow-down"></i></button>';
            } elseif ($item->type == "article") {
                $sub_array['action'] = '<button class="btn btn-primary mr-2" onclick="window.location.href=\'' . url('training-room/article/edit/' . $item->id . '/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId) . '\'" data-toggle="tooltip" title="Edit"><i class="fas fa-edit"></i></button><button class="btn btn-primary mr-2" id="delete_' . $item->id . '" onclick="deleteTrainingRoom(this.id);" data-toggle="tooltip" title="Delete"><i class="fas fa-trash"></i></button><button class="btn btn-primary mr-2" id="copy_' . $item->id . '" onclick="copyTrainingRoomItem(this.id);" data-toggle="tooltip" title="Copy"><i class="fas fa-copy"></i></button><button class="btn btn-primary mr-2" id="orderUp_' . $item->id . '" onclick="window.location.href=\'' . $Url . '\';" data-toggle="tooltip" title="Order Up"><i class="fas fa-arrow-up"></i></button><button class="btn btn-primary mr-2" id="orderDown_' . $item->id . '" onclick="window.location.href=\'' . $Url2 . '\';" data-toggle="tooltip" title="Order Down"><i class="fas fa-arrow-down"></i></button>';
    ...
    public function TrainingRoomDelete(Request $request)
    {
        ...
        if ($affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Record has been deleted from the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    function TrainingRoomCopy(Request $request)
    {
        ...
        if ($Affected) {
            DB::commit();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('message', 'Record has been copy from the training room successfully');
        } else {
            DB::rollback();
            return redirect(url('training-room/folder/details/' . $TrainingRoomFolderId . '/' . $TrainingRoomRoleId))->with('error', 'Error! An unhandled exception occurred');
        }
    ...
    function TrainingRoomOrderUp($Id, $FolderId, $Role){
        ...
        return redirect(url('training-room/folder/details/' . $FolderId . '/' . $Role))->with('message', 'Record order changed successfully');
    }
    ...
    public function SearchCourse(Request $request)
    {
        ...
        $AllTrainingAssignmentFolders = DB::table('training_assignment_folders')
